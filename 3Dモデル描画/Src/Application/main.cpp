...
void Application::Execute()
{
	...
	while (1)
	{
		...
		// ゲーム描画
		Math::Color color(1.0f,0.5f,1.0f,1);

		D3D.WorkDevContext()->ClearRenderTargetView(D3D.WorkBackBuffer()->WorkRTView(),color);									//ずっとお世話になる大事なコード

		D3D.WorkDevContext()->ClearDepthStencilView(D3D.WorkZBuffer()->WorkDSView(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);//目に見えない大事な処理
		
		KdCamera came;//カメラの名前
		came.SetProjectionMatrix(60.0f);

		Math::Matrix cameMat = Math::Matrix::CreateTranslation(0.0f, 3.0f, -5.0f);//別のカメラのセット

		came.SetCameraMatrix(cameMat);//仮想カメラの位置を与える

		came.SetToShader();

		KdModelData made;
		made.Load("Asset/Models/Terrain/Terrain.gltf");
		
		D3D.WorkShaderManager().m_HD2DShader.BeginNoLighting();//描画の設定（スタート）

		D3D.WorkShaderManager().m_HD2DShader.DrawMesh(made.GetMesh(0).get(),Math::Matrix::Identity,made.GetMaterials(),kWhiteColor);

		D3D.WorkShaderManager().m_HD2DShader.EndNoLighting();//描画の設定（フィニッシュ）
																													   
		// BackBuffer -> 画面表示										　インテリセンス(検索候補)を出すにはCtrl+SPACE
